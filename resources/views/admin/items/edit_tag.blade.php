...
@section('admin-content')
  ...
  <p>Edit the parameters for this item tag on this item. Note that for the item tag to take effect (e.g. become a usable
    item), you will need to turn on the Active toggle. (Conversely, you can turn it off to prevent users from using it
  ...
  <script>
    $(document).ready(function() {
      $('.delete-tag-button').on('click', function(e) {
        e.preventDefault();
        loadModal("{{ url('admin/data/items/delete-tag') }}/{{ $item->id }}/{{ $tag->tag }}",
          'Delete Tag');
      });
    });
  </script>
